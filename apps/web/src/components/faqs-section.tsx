    ...
    <div className='mx-auto min-h-screen w-full max-w-3xl md:border-x'>
      <div className='px-4 py-16 lg:px-6'>
        ...
        <p className='mb-8 max-w-2xl text-muted-foreground'>
          Find answers to common questions about Efferd. Can't find what you're looking for? Our support team is here to
          help.
        </p>
      ...
      <div className='flex flex-wrap gap-1 border-b px-4 md:gap-3'>
        {categories.map((cat) => (
          <button className='flex flex-col' key={cat.id} onClick={() => setActiveCategory(cat.id)} type='button'>
            <span
              className={cn(
      ...
      <Accordion className='space-y-2 px-4 py-12 lg:px-6' collapsible defaultValue='item-1' type='single'>
        {filtered.map((faq) => (
          <AccordionItem
          ...
          >
            <AccordionTrigger className='px-4 hover:no-underline focus-visible:ring-0'>{faq.title}</AccordionTrigger>
            <AccordionContent className='px-4 pt-2 pb-4 text-muted-foreground'>{faq.content}</AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
