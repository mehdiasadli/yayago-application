...
export function useSwipeRowContext() {
  ...
  if (!ctx) throw new Error('Swipe Row components must be used inside <SwipeRow>');
  return ctx;
}
...
export function SwipeRow({ className, children }: React.ComponentProps<'div'>) {
  ...
  const handleDragEnd = React.useCallback((_: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    ...
    if (actionRefLeft.current && absoluteOffset > DRAG_THRESHOLD && info.offset.x > 0) {
      setDragX(actionRefLeft.current.offsetWidth);
    } else if (actionRefRight.current && absoluteOffset > DRAG_THRESHOLD && info.offset.x < 0) {
...
export function SwipeRowContent({ children, className, ...props }: HTMLMotionProps<'div'>) {
  const { actionRefLeft, actionRefRight, dragX, handleDrag, handleDragEnd } = useSwipeRowContext();
  ...
  return (
    <motion.div
      ...
      drag='x'
      dragConstraints={{
        left: actionRefLeft?.current ? -actionRefLeft?.current?.offsetWidth || 0 : 0,
...
export function SwipeLeftActions({ className, children, ...props }: HTMLMotionProps<'div'>) {
  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        x: dragX > 0 && actionRefLeft ? 0 : -(actionRefLeft?.current?.offsetWidth || 0),
      }}
      transition={{ stiffness: 300 }}
      {...props}
    >
...
export function SwipeRightActions({ className, children, ...props }: HTMLMotionProps<'div'>) {
  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        x: dragX < 0 && actionRefRight ? 0 : actionRefRight?.current?.offsetWidth || 0,
      }}
      transition={{ stiffness: 300 }}
      {...props}
    >
