...
export function PricingSection({ plans, heading, description, ...props }: PricingSectionProps) {
  ...
  return (
    <div className={cn('flex w-full flex-col items-center justify-center space-y-7 p-4', props.className)} {...props}>
      <div className='mx-auto max-w-xl space-y-2'>
        <h2 className='text-center font-bold text-2xl tracking-tight md:text-3xl lg:font-extrabold lg:text-4xl'>
          {heading}
        </h2>
        {description && <p className='text-center text-muted-foreground text-sm md:text-base'>{description}</p>}
      </div>
...
export function PricingCard({ plan, className, frequency = 'monthly', ...props }: PricingCardProps) {
  return (
    <div
      ...
      {...props}
    >
      <div className={cn('rounded-t-lg border-b p-4', plan.highlighted && 'bg-card dark:bg-card/80')}>
        ...
        <h3 className='mt-6 mb-2 flex items-end gap-1'>
          <span className='font-extrabold text-3xl'>${plan.price[frequency]}</span>
          <span className='text-muted-foreground'>
            {plan.name !== 'Free' ? `/${frequency === 'monthly' ? 'month' : 'year'}` : ''}
          </span>
        </h3>
      </div>
      <div className={cn('space-y-4 px-4 pt-6 pb-8 text-muted-foreground text-sm', plan.highlighted && 'bg-muted/10')}>
      ...
      <div className={cn('mt-auto w-full rounded-b-lg border-t p-3', plan.highlighted && 'bg-card dark:bg-card/80')}>
